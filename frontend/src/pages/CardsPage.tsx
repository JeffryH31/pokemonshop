      ...
      <div className="mt-6 relative">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-[#c2bcb0] font-medium">Gagal memuat produk</p>
            <p className="text-sm text-[#c2bcb0] mt-1">Periksa koneksimu lalu coba lagi</p>
                  ...
                  {Array.from({ length: Math.min(data.last_page, 7) }, (_, i) => {
                    ...
                    if (data.last_page > 7) {
                      if (page > 3 && page < data.last_page - 2 && Math.abs(page - current) > 1) {
                        if (page === 4) return <span key={page} className="text-[#c2bcb0] px-1">…</span>
                        return null
                      }
                    }
                    return (
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg text-sm font-medium transition-all cursor-pointer ${
                          page === current
                            ...
                            : 'text-[#c2bcb0] hover:bg-[#1c1c28] hover:text-[#f0ece4]'
                        }`}
                      >
                        {page}
                      </button>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-16 h-16 rounded-full bg-[#1c1c28] flex items-center justify-center mb-4">
              <Layers size={24} className="text-[#c2bcb0]" />
            </div>
            <p className="text-[#c2bcb0] font-medium">Produk tidak ditemukan</p>
            <p className="text-sm text-[#c2bcb0] mt-1">Coba ubah filter pencarian kamu</p>
            <Button
              variant="ghost"
              className="mt-4"
